...
подпись и presign в `:s3-sigv4`. Приёмка: 34 официальных вектора для общего SigV4 плюс
20 сгенерированных из botocore для правил S3 и presign (`docs/spec/s3-signing-vectors/`).
Открыт весь раздел 4: это целевое поведение, ни один запрос ещё не отправляется.
