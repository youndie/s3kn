...
Статус реализации: закрыты разделы 1 (вывод `host`), 2 (кодирование ключа) — модуль `:s3-core` —
и 3.1–3.4 в **общем** режиме SigV4, проверенные всеми 34 официальными векторами — модуль
`:s3-sigv4`. Открыто: режим S3 в 3.1 (путь как есть), 3.4 (`x-amz-content-sha256`), 3.5 (presign)
и весь раздел 4. Это целевое поведение, кода под ним пока нет.
...
Последняя строка таблицы — не мелочь: `🙂` в Kotlin-строке это суррогатная пара, и кодировщик,
идущий по `Char`, выдаст на ней два символа замены и подпишет не то, что отправит.
...
**Ошибка тоже без тела.** `404` не несёт XML с `<Code>NoSuchKey</Code>`; код выводится из статуса.
Модель ошибки обязана это допускать (ресёрч, Риск 7).
