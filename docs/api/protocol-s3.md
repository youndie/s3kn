...
Статус реализации: закрыты разделы 1 (вывод `host`), 2 (кодирование ключа) и строка
`CANONICAL_QUERY` из 3.1 — модуль `:s3-core`. Всё остальное — целевое поведение, выведенное из
спецификации, кода под ним пока нет.
